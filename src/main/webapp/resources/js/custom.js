...
// Scroll to top
/*
 * var $block =$('<div/>',{'class':'top-scroll'}).append('<a
 * href="#"/>').hide().appendTo('body').click(function () {
...
$("#search-sort").selectbox(
		{
			onChange : function(val, inst) {
				...
				$(inst.input[0]).find('[value="' + val + '"]').attr('selected',
						'selected');
			}
...
function init_Elements() {
	...
	$("#select-sort").selectbox(
			{
				onChange : function(val, inst) {
					...
					$(inst.input[0]).find('[value="' + val + '"]').attr(
							'selected', 'selected');
				}
	...
	$(document).click(
			function(e) {
				var ele = $(e.target);
				if (!ele.hasClass("datepicker__input")
	...
	$('.mega-select__filter').click(function(e) {
		...
		// get the data attribute of the clicked link(which is equal to value
		// filter of our content
...
function init_BookingOne() {
	...
	// var for booking;
	var movie = $('.choosen-movie'), city = $('.choosen-city'), date = $('.choosen-date'), cinema = $('.choosen-cinema'), time = $('.choosen-time');
	...
	$('.time-select__item').click(
			function() {
				...
				$('.choose-indector--time').find('.choosen-area').text(
						chooseTime);

				// data element init
				var chooseCinema = $(this).parent().parent().find(
						'.time-select__place').text();
...
function init_BookingTwo() {
	...
	// 2. Init vars for order data
	// var for booking;
	var numberTicket = $('.choosen-number'), sumTicket = $('.choosen-cost'), cheapTicket = $('.choosen-number--cheap'), middleTicket = $('.choosen-number--middle'), expansiveTicket = $('.choosen-number--expansive'), sits = $('.choosen-sits');
					...
					function(e) {
						...
						if (!$(e.target).hasClass('sits-state--your')) {
							...
							if (!$(this).hasClass('sits-state--not')) {
								...
								/*
								 * switch (ticketPrice) { case '10': sum += 10;
								 * cheap += 1; break; case '20': sum += 20;
								 * middle += 1; break; case '30': sum += 30;
								 * expansive += 1; break; }
								 */
								if (typeof (Storage) !== "undefined") {
									seats.push(place);
							...
							/*
							 * switch (ticketPrice) { case '10': sum -= 10;
							 * cheap -= 1; break; case '20': sum -= 20; middle -=
							 * 1; break; case '30': sum -= 30; expansive -= 1;
							 * break; }
							 */
							if (typeof (Storage) !== "undefined") {
	...
	function ChoosePlace(e) {
		...
		var row = $(this).parent().find('.sit-row option[selected="selected"]')
				.text();
		var number = $(this).parent().find(
				'.sit-number option[selected="selected"]').text();
		var ch_sits = row + number;
		var ticketPrice = 0;

		if ($('.checked-place').find(
				".choosen-place[data-sit='" + ch_sits + "']").length) {
			alert('same place');
			return 0;
		}

		$('.sits-area--mobile .checked-place').prepend(
				'<span class="choosen-place" data-sit="' + ch_sits + '">'
						+ ch_sits + '</span>');

		/*
		 * if (row == "A" || row == "B" || row == "C" || row == "D") {
		 * ticketPrice = 10; } else if (row == "E" || row == "F" || row == "G" ||
		 * row == "I") { ticketPrice = 20; } else if (row == "J" || row == "K" ||
		 * row == "L") { ticketPrice = 30; }
		 */

		/*
		 * switch (ticketPrice) { case 10: sum += 10; break; case 20: sum += 20;
		 * break; case 30: sum += 30; break; }
		 */
		sum += parseFloat(ticketPrice);
		...
		$(this).click(
				function(e) {
					...
					var row = $(this).parent().find(
							'.sit-row option[selected="selected"]').text();
					var numbers = $(this).parent().find(
							'.sit-number option[selected="selected"]').text();
					...
					if (activeSit.length) {
						...
						/*
						 * switch (ticketPrice) { case 10: sum -= 10; break;
						 * case 20: sum -= 20; break; case 30: sum -= 30; break; }
						 */
...
function init_Contact() {
	...
	// Creates the pop-up infobox for Glastonbury, adding the configuration
	// options set above.
	var infoboxA = new InfoBox(infoboxOptionsA);

	// Add an 'event listener' to the Glastonbury map marker to listen out for
	// when it is clicked.
...
function init_MovieList() {
			...
			.selectbox(
					{
						onChange : function(val, inst) {
							...
							$(inst.input[0]).find('[value="' + val + '"]')
									.attr('selected', 'selected');
							var maturityVal = null;
							switch (val) {
							case '1':
								maturityVal = 'G';
								break;
							...
							window.location.href = 'http://localhost:8085/OnlineMovieTicketManagement/movielist?maturity='
									+ maturityVal;
						}
					});
			...
			.selectbox(
					{
						onChange : function(val, inst) {
							...
							$(inst.input[0]).find('[value="' + val + '"]')
									.attr('selected', 'selected');
							window.location.href = 'http://localhost:8085/OnlineMovieTicketManagement/movielist?genre='
									+ val;
						}
					});
	...
	$('.movie__show-btn1').click(
			function(ev) {
				ev.preventDefault();

				$(this).parents('.movie--preview').find('.time-select')
						.slideToggle(500);
				if (toggleFlag == 2) {
					$(this).parents('.movie--preview').find('.time-select')
							.slideToggle(500);
...
function init_MoviePage() {
	...
	var map = new google.maps.Map(document.getElementById('cimenas-map'),
			mapOptions);
	var marker = new google.maps.Marker({
		map : map,
		position : map.getCenter()
...
function init_SinglePage() {
	...
	var mySwiper = new Swiper('.swiper-container', {
		...
		onSlideChangeStart : function change(index) {
			...
			var prevSlide = $('.post__preview .swiper-slide').eq(prev).attr(
					'data-text');
			$('.arrow-left').find('.slider__info').text(prevSlide);
			var nextSlide = $('.post__preview .swiper-slide').eq(next).attr(
					'data-text');
	...
	var prevSlide = $('.post__preview .swiper-slide').eq(prev)
			.attr('data-text');
	var nextSlide = $('.post__preview .swiper-slide').eq(next)
			.attr('data-text');
...
function init_Trailer() {
	...
	$('.trailer-btn').click(
			function(e) {
				...
				$(this).hide();
				$(this).parent().addClass('trailer-block--short').find(
